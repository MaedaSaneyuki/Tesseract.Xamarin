...
namespace Tesseract.Test.Droid
{
    public class Converter
    {
        ...
        public static int YLevelBorder = 215;
        ...
        {
            for(var ite =0;ite< previewSize.Width * previewSize.Height; ite +=2)
            {
                if(yuvData[ite]<YLevelBorder)
                {
                    yuvData[ite] = 255;
                    yuvData[ite + 1] = 255;
                  //  yuvData[ite + 2] = 0;
            ...
            for (var ite = previewSize.Width * previewSize.Height; ite < yuvData.Length; ite++)
            {
                yuvData[ite] = 130;
            }
            ...
            try
            {
             ...
             //   m.PostRotate(dr);
               ...
               // m.PostTranslate(dbX, dbY);
                ...
                var interpret = Bitmap.CreateBitmap(bmp, (int)left, (int)top, centerClip.Width, centerClip.Height);
                //ToInterpret = bmp.Copy(bmp.GetConfig(), true);
                ToInterpret = interpret;
                using (var ms = new MemoryStream())
                {
                    var quality = 100;
